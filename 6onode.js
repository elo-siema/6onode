...
var szObcyClient = function(port, hub){
  ...
  //
  var circumventSecurity = function(msg) {
	  msg = msg.replace(" cb", " ciebie");
	  msg = msg.replace(" lol", " haha");
	  msg += " ";
	  switch(Math.round(Math.floor()*3)) {
		  case 0:
			msg = msg.toLowerCase();
			break;
		  case 1:
  ...
  var emitMessage = function(name, data) {
    ...
    switch(name) {
      ...
      case '_pmsg': //post message
        ...
        if(this.conversation.idn == null){
          e = '{"ev_name":"_pmsg","ev_data":{"ckey":"'+ this.conversation.ckey +'", "msg":"'+circumventSecurity(data)+'", "idn":0},"ceid":'+ ceid +'}';
          this.conversation.idn = 0;
        }
        else{
          e = '{"ev_name":"_pmsg","ev_data":{"ckey":"'+ this.conversation.ckey +'", "msg":"'+circumventSecurity(data)+'", "idn":'+ this.conversation.idn +'},"ceid":'+ ceid +'}';
        }
        ceid = ceid +1;
    ...
    setTimeout(function(){sockjs.send(JSON.stringify(JSON.parse(e)));}, timeout);
  };
